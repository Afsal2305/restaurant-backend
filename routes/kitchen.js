...
router.post('/generate-kot', authenticateAdmin, async (req, res) => {
  try {
    const { order_id, table_id, table_number, waiter_id, waiter_name, order_type, items, notes } = req.body;
    if (!items || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ error: 'Items are required for KOT' });
    }
    ...
    for (const item of items) {
      await pool.query(
        ...
        [kotId, item.menu_item_id, item.item_name, item.quantity, item.price, item.notes || null, item.variant_name || null, item.addon_names || null]
      );
    }
...
router.put('/:id/status', async (req, res) => {
  try {
    const { status, chef_username, chef_code, chef_name } = req.body;
    ...
    if (userRole === 'chef') {
      if (status === 'preparing') {
        // Chef identification required to take ownership
        ...
        // Chef must be the assigned chef for other transitions
...
router.get('/stats', async (req, res) => {
  try {
    ...
    const [all] = await pool.query('SELECT COUNT(*) as count FROM kot');
    res.json({
      all: all[0].count,
