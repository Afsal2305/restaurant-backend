...
router.post('/chef/login', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      "SELECT * FROM waiters WHERE username = ? AND role = 'chef'",
      [username]
    );
    if (rows.length === 0) {
      return res.status(401).json({ error: 'Invalid credentials' });
    ...
    const token = jwt.sign(
      { id: chef.id, name: chef.name, username: chef.username, chef_code: chef.chef_code, role: 'chef' },
      process.env.JWT_SECRET,
      { expiresIn: '24h' }
    );
    res.json({ token, user: { id: chef.id, name: chef.name, username: chef.username, chef_code: chef.chef_code, role: 'chef' } });
  } catch (error) {
